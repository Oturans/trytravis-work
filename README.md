...
2. На практических примерах расмотрели как влияет очередность создания ресурсов, от внутренних зависимостей одного ресурвы от другого. Далее в ДЗ это будет примеренно. при разворачивании приложения.
...
4. Вынесли из основного файла приложение, база и настройки сети в отдельные модули:  
      ./terraform/module/app  - приложение  
      ./terraform/module/db   - база данных  
      ./terraform/module/vpc  - настройки сети  
5. Создали 2 отдельных набора сред stage и prod с ипользованием ранее созданных модулей(пункт 4) полностью перенеся внутрю каждой из сред ранее настроенное. Отличие prod среды от stage это ограничение доступа к prod только с локального ip. 
...
8. С помошью Provisioner проведен деплой приложений
   reddit-app:  
    ```
        provisioner "file" {
